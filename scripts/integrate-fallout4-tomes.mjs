...
const businessRules = {
  ...
  playableCounts: {
    ...
    weapons: templates.counts?.weapons ?? templates.catalogs?.weapons?.length ?? 0,
    equipment: templates.counts?.equipment ?? templates.catalogs?.equipment?.length ?? 0,
    settlements: templates.counts?.settlements ?? templates.catalogs?.settlements?.length ?? 0,
    factions: templates.counts?.factions ?? templates.catalogs?.factions?.length ?? 0,
    collectibles: templates.counts?.collectibles ?? templates.catalogs?.collectibles?.length ?? 0,
    survival: Object.keys((await readJson("features/supervivencia/config.json", {})).rules ?? {}).length || 1,
    economy: Object.keys((await readJson("features/asentamientos/config.json", {})).economy ?? {}).length || 1,
    weather: locations.counts?.weather ?? 4,
